...
public class IntQueueGeneric<E> {
    ...
    public static class AlreadyInUseException extends RuntimeException {
    	public AlreadyInUseException() { }
    }
    
    public int setPosition(int i) throws AlreadyInUseException, OverflowIntQueueException {
    	if (num != 0) 
    	{
    ...
    public int search(E x) {
    	...
    	int idx = indexOf(x);
    	if (idx >= 0) {
    		idx = idx - front > 0 ? idx - front : (capacity - front) + idx ;  
    		return idx;
        }
        return 0;                            // 검색 실패
    }
